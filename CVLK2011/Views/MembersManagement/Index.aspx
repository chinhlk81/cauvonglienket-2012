...
//    }
    function UserFilter() {
        ...
        $.post('/MembersManagement/FilterMembers',
            { userType: userType, userGender: userGender, userStatus: userStatus, eventParrent: event, accountType: accountType },
            //$(this).serialize(),
            function (data) {
...
<div style="text-align:right">
    ...
    <br />
<%} %>
<%using (Html.BeginForm("DeleteMultiUserAction", "MembersManagement", new { box = Model.box }, FormMethod.Post))
{ %>
<div style="text-align:right">
...
<%} %>--%>
 <div><b><%=Html.ActionLink("Quản lý sinh nhật", "Index", "AdminHome")%></b></div>
 <div><b><%=Html.ActionLink("Quản lý Avatar", "NewAvatarListView", "MembersManagement")%></b></div>
<%=Html.DropDownList("accountType", ViewData["AccountType"] as SelectList, "--Loại tài khoản--", new { onchange = "UserFilter()" })%>
    ...
    <%if (Session["Type"] != null)
      {
          ...
          { %>
        <input type="submit" name="Delete" value="Xóa" class="btn_user_update" onclick="return ConfirmDelete()"/> 
        <%}
      }%>
</div>
<div id="userList">
...
Tổng Số Thành viên: <%=ViewData["UserQty"]%>
<%
} %>
</asp:Content>
