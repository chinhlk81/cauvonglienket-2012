...
if (Model != null)
{
    ...
    { %>
        <tr align="left">
            ...
            <td >
             <%if (item.CommentContent.Length > 80)
                      { %>
                       
                    <%= item.CommentContent.Remove(80)%>...
                    <%}
                      else
                      { %>
                       <%=item.CommentContent%>
...
<div class="pager">        
    ...
    x => Url.Action("MembersComments", new {datePost = Model.commentDateFilter, status = Model.commentStatusFilter, page=x, comment = Model.CurrentComment})) %>
    </div>
